...
// [[Rcpp::export]]
List mcmc_chain_pjnormar1_group(arma::mat sphericaldata, arma::uvec protein_first, arma::mat b_mat, int iterations=5000){
  //dimension
  ...
  arma::vec ind1 = ind;
  ind1(0) = H;
  ind1.elem(protein_first).fill(H);
  ...
  arma::cube phi_arr(k,k,(H+1)); phi_arr.zeros();
...
//  arma::arma_rng::set_seed(seed);
  ...
  {
    //sampling mu
    subtracted_b_cube = subtract_b_cube(b_cube, phi_arr, ind1, n);
    subtracted_y = subtract_y(phi_arr, ind1, augmenteddata_raw, n);
    ...
    //sampling for r
    for(nn=0; nn<n; nn++){
     r_present(nn) = gen_r_group(ind, ind1, mu_present_mat, precision_full_arr,
